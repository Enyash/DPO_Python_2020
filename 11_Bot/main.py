...
def stat(tag=0):
    ...
    df = pd.DataFrame(d)[['Country,Other', 'TotalCases', 'NewCases', 'TotalDeaths', 'NewDeaths',
...
def cmd_listfields(message):
    x = list(stat()[['TotalCases', 'NewCases', 'TotalDeaths', 'NewDeaths',
                     'TotalRecovered', 'ActiveCases', 'SeriousCritical', 'TotalTests']].columns)
...
def enter_country_or_region(message):
    ...
    countries = [i.strip() for i in re.findall(r'[\w\s]+', message.text)]

    country = int(dbworker.get_current_state(str(message.chat.id)+'country'))
    ...
    if errors == []:
        ...
        bot.send_message(message.chat.id, "There\'s a number of countries/regions with typos or something that\'s bot in our list.\n"
